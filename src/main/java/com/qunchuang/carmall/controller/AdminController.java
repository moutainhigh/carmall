...
public class AdminController {
    ...
    public Admin platformAdministrator(@DomainRule("phone && name && password && username") Admin admin) {
        return adminService.platformAdministrator(admin);
    }

    @SchemaDocumentation("添加销售顾问")
    @GraphqlMutation(path = "/salesConsultant")
    public Admin salesConsultant(@DomainRule("phone && name && password && username") Admin admin) {
        return adminService.salesConsultant(admin);
    }

    @SchemaDocumentation("添加门店管理员")
    @GraphqlMutation(path = "/storeAdministrator")
    public Admin storeAdministrator(@DomainRule("phone && name && password && username") Admin admin) {
